...
export default class StarReview extends Component {
  ...
  halfRatingMode() {
    ...
    for (let i = 1; i < this.props.count + 1; i++) {
      const star =
        i <= this.state.rating ? this.props.fullStar : this.props.emptyStar;
      const halfStar =
        this.state.rating + 0.5 == i ? this.props.halfStar : null;
...
StarReview.propTypes = {
  ...
  emptyStar: PropTypes.oneOfType([PropTypes.number, PropTypes.object])
    .isRequired,
  fullStar: PropTypes.oneOfType([PropTypes.number, PropTypes.object])
    .isRequired,
